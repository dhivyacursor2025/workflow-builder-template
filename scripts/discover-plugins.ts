...
function generateCodegenRegistry(): void {
  ...
  if (entries.length === 0) {
    ...
    // Still generate the file with empty exports to avoid import errors
    const emptyContent = `/**
...
`;
    writeFileSync(CODEGEN_REGISTRY_FILE, emptyContent, "utf-8");
    return;
  }
...
/**
 * Pre-generate types file with all plugin types from directory names
 * This must run BEFORE importing plugins to avoid circular dependency
 */
function preGenerateTypesFile(plugins: string[]): void {
  // Ensure the types directory exists
  const typesDir = dirname(TYPES_FILE);
  if (!existsSync(typesDir)) {
    mkdirSync(typesDir, { recursive: true });
  ...
  // Combine plugin types (folder names) with system types
...
`;
  ...
  writeFileSync(TYPES_FILE, content, "utf-8");
  console.log(
    `Pre-generated lib/types/integration.ts with ${allTypes.length} type(s)`
  );
}
...
async function main(): Promise<void> {
  ...
  // Pre-generate types file FIRST to avoid circular dependency
  console.log("\nPre-generating lib/types/integration.ts...");
  preGenerateTypesFile(plugins);
